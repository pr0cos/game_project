...
def boss_fight(score):
    ...
    pygame.time.set_timer(START_LASER_HIDE, 5500)
    bg = pygame.sprite.Group()
    pl = pygame.sprite.Group()
        ...
        if not paused:
            for event in pygame.event.get():
                ...
                if event.type == CIRCLE_ATTACK:
                    rect_x = bs.rect.x + bs.rect.w // 2
                    rect_y = bs.rect.y + bs.rect.h // 2
            ...
            screen.fill((0, 0, 0))
            bg.draw(screen)
            pygame.draw.rect(screen, (255, 255, 255), ((width - 204) // 2, 10, 204, 20), 2)
            pygame.draw.rect(screen, (255, 0, 0), ((width - 204) // 2 + 2, 12, bs.hp * 4, 16))
            if pygame.key.get_pressed()[pygame.K_LEFT] and player.rect.x > 0:
                player.rect.x -= v_player / fps
            if pygame.key.get_pressed()[pygame.K_RIGHT] and player.rect.x + player.rect.w < width:
                player.rect.x += v_player / fps
